...
class Potassium_TestCase(_Material_Test, unittest.TestCase):
    ...
    def test_pseudoDensity(self):
        cur = self.mat.pseudoDensity(Tk=333)
        ref = 0.828
        delta = ref * 0.001
        self.assertAlmostEqual(cur, ref, delta=delta)

        cur = self.mat.pseudoDensity(Tk=500)
        ref = 0.7909
        delta = ref * 0.001
        self.assertAlmostEqual(cur, ref, delta=delta)

        cur = self.mat.pseudoDensity(Tk=750)
        ref = 0.732
        delta = ref * 0.001
        self.assertAlmostEqual(cur, ref, delta=delta)
...
class UraniumOxide_TestCase(_Material_Test, unittest.TestCase):
    ...
    def test_densityTimesHeatCapactiy(self):
        Tc = 500.0
        expectedRhoCp = (
            self.mat.pseudoDensity(Tc=Tc) * 1000.0 * self.mat.heatCapacity(Tc=Tc)
        )
...
class Void_TestCase(_Material_Test, unittest.TestCase):
    ...
    def test_pseudoDensity(self):
        ...
        this material has no density function
        """
        self.assertEqual(self.mat.pseudoDensity(500), 0)
